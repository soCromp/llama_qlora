...
class MultiheadLlamaForCausalLM(LlamaForCausalLM):
    ...
    def generate(
        ...
        **kwargs,
        ...
        try:
            do_mlm_sample = kwargs.pop('do_mlm_sample')
        except:
            do_mlm_sample = False
        ...
        **model_kwargs,
        ...
        for i in range(input_ids.shape[0]):
            for h,j in enumerate(locs_tok):
                # print(i, j.item(), input_ids[i,j.item()])
                logits = self.heads[h](hidden_states[:, :j.item(), :])
                ...
                # argmax
                next_tokens = torch.argmax(next_tokens_scores, dim=-1) # batch_size x j
                result[:, j.item()] = next_tokens[:, -1]
                # print(result)
                # resultslist.append(copy.deepcopy(result))
